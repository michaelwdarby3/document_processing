...
class StubDoc:
    def __init__(self, tables, page_no: int = 1) -> None:
        self.tables = tables
...
def test_write_table_metadata(tmp_path: Path) -> None:
    table = StubTable(
        bbox=StubBBox(l=10, t=50, r=60, b=0),
        page_no=1,
        cells=[
            StubCell(
                bbox=StubBBox(l=10, t=40, r=20, b=30),
...
def test_export_tables_to_xlsx(tmp_path: Path) -> None:
    table = StubTable(
        ...
        cells=[
            StubCell(
                bbox=StubBBox(l=0, t=10, r=10, b=0),
                start_col_offset_idx=0,
                end_col_offset_idx=1,
                text="A",
            ),
            StubCell(
                bbox=StubBBox(l=10, t=10, r=20, b=0),
