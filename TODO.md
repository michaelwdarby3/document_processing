...
- [ ] Containerize the tool once you need production deployment. Start from a base like `python:3.11-slim`, install the repo’s `requirements.txt`, add GPU extras (CUDA runtime / `libgl1`, etc.), install system OCR binaries if you plan to use Tesseract, and bake in the `.artifacts/` cache for true offline runs.
